                    ...
                    for attempt in range(LAST_CALLED_RETRY_LIMIT + 1):
                        ...
                        try:
                            async with account_live["last_called_api_lock"]:
                                ...
                                # Keep the history fetch inside last_called_api_lock so
                                # the network call stays serialized with service/global
                                # refreshes that share the lock; otherwise the per-account
                                # rate-limit protection is defeated and 429/backoff become
                                # more likely. The records processing below runs after the
                                # lock is released (kept as short as the serialization needs).
                                try:
                                    records = (
                                        await AlexaAPI.get_customer_history_records(
                                            login_live,
                            ...
                            # Known alexapy/aiohttp edge case (fork PR #22): an expired
                            # CSRF token can yield a None "anti-csrftoken-a2z" header,
                            # crashing aiohttp's header writer with TypeError. Re-arm with
                            # backoff instead of letting it kill the probe worker.
                            account_live["last_called_probe_next_allowed"] = (
                                time.monotonic() + LAST_CALLED_CONN_BACKOFF_S
                            )
                            _LOGGER.warning(
                        ...
                        if not payload:
                            ...
                            if trigger_cmd in (
                                ...
                                try:
                                    ...
                                    # Known alexapy/aiohttp None-CSRF-header crash (fork
                                    # PR #22): log and recover instead of killing the worker.
                                    _LOGGER.warning(
                                        "%s: fallback last_called refresh TypeError (%s): %s",
                                        hide_email(email),
                                        trigger_cmd,
                                        exc,
                                        exc_info=True,
